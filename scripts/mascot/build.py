...
# Generated alpha can contain nearly invisible specks far outside a bust.
# Measure visible content so those specks cannot shrink an entire color variant.
def visible_box(frame):
    return frame.getchannel('A').point(lambda a: 255 if a > 20 else 0).getbbox()


top = min(visible_box(f)[1] for f in frames) - 8
left = min(visible_box(f)[0] for f in frames)
right = max(visible_box(f)[2] for f in frames)
...
content_bottom = max(visible_box(f)[3] for f in frames)
